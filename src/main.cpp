...
#define VERSION "V2.1"                                    // just the PvF version
const char version[] = VERSION " " __DATE__;              // PvF prefers to have the last compilation time
...
#include "utils.h"                                        // Some handy utilities
  ...
  #define dimPin 39
  #define keyPin 35
  #define NoAnalogButtonPressed 4000 /* actually 4095; maximum value ADC reading connected to 3.3V */
  
  FidelioDisplay fideDisplay;
#endif

/* Philips Fildelio Resistance Key Pin values measured with 3kohm pullup to 3.3v
1. clock :    0 -  100
2. +     :  470 -  530
3. light : 1060 - 1120
4. -     : 2220 - 2280
not pressed anything bigger then 2280
*/
...
void        displayStop();
bool        getPresetName (int16_t presetId, String &str);  // check if icy or preset name shall be used
#ifdef SCANANALOG
void        scananalog(); // scaning Philips
#endif
...
struct ini_struct
{
  ...
  int8_t         pin_led_out;                         // PvF: GPIO out Pin for bottom LED driver
  int8_t         pin_stb_out;                         // PvF: GPIO out STB  Fidelo Display PT6964
  int8_t         pin_clk_out;                         // PvF: GPIO out CLK  Fidelo Display PT6964
  int8_t         pin_dio_out;                         // PvF: GPIO out DI/O Fidelo Display PT6964
...
enum enc_menu_t { VOLUME, PRESET, TRACK };    // State for rotary encoder menu, OFF for new toggle comand
...
progpin_struct   progpin[] =                             // Input pins and programmed function
{
  ...
  //{ 35, true, false,  "", false },                     // Fidelio 4 keys resitor based
  //{ 36, true,  false,  "", false },                    // Reserved for ADC battery level
  //{ 39, true,  false,  "", false },                    // Light sensor for Fidelio Display
...
#ifdef SCANANALOG
// PvF analog scan structure
struct analogpin_struct                                 // PvF: For programmable analog input pin
{                                                       //      of resitor ladder
  int8_t        commandNumber;                          // Pin number GPIO
  uint16_t      minValue ;                              // min value detected by pressed button
  bool          avail ;                                 // Pin is available for a command
  String        command ;                               // Command to execute when activated
                                                        // Example: "uppreset=1"
  bool          cur ;                                   // Current state, true = HIGH, false = LOW
  int8_t        last_cmd;                               // Counter number of times low level
} ;

#define NumButtons 4  /* excluding the last one status for unpressed */
analogpin_struct   analogpin[] =              // Touch pins and programmed function
// All pins are not available as long as they are not defined in the preferences
{
  {   0,  100, false, "", false, 0 },                   // clock    
  {   1,  800, false, "", false, 0 },                   // +
  {   2, 1500, false, "", false, 0 },                   // light
  {   3, 2500, false, "", false, 0 },                   // -
  {  -1,    0, false, "", false, 0 }                    // End of list
  // End of table
} ;
#endif

// Actualy not only for Fidelio, can be used for any connetced PWM LED 
// bottom leds PWM value                                 // Fidelio bottom LEDs
#define MAXPWMIDX 4
u_int8_t PWMValue[] = {0, 40, 128, 255};                 // possible LED PWM values
u_int8_t PWMidx = 0;                                     // brightnes' index 
...
//**************************************************************************************************
//                                     G E T  P R E S E T  N A M E                                 *
//**************************************************************************************************
// Extract name from the preset string                                                             *
//**************************************************************************************************
 bool getPresetName (int16_t presetId, String &str) {
  bool defaultName = false;
  int16_t inx;

  str = readhostfrompref ( presetId ) ;                     // Get host spec and possible comment        
  if (str != "") {
    inx = str.indexOf ( "#" ) ;                             // Get position of "#"
    if ( inx > 0 ) {                                        // Hash sign present?
      if (str.charAt(inx+1) == '!') {
        inx += 1;
        defaultName = true;                                 // ! defualt name detected
      }
      str.remove ( 0, inx + 1 ) ;                           // Yes, remove non-comment part
    }
    chomp ( str ) ;                                         // Remove garbage from description
  }
  return defaultName;
}
...
bool connecttohost()
{
  ...
  char        getreq[500] ;                          // GET comand for MP3 host
  ...
  if ( mp3client->connect ( hostwoext.c_str(), port ) )
  {
    ...
    #ifdef FIDELO
      fideDisplay.alarm(false);                           // PvF: in Stop mode turn on alarm LED
    #endif    
...
#ifdef SCANANALOG
//**************************************************************************************************
//                                   R E A D A N A L O G B U T T O N S                             *
//**************************************************************************************************
// Read the preferences for the analog / resitor based one line connected butons                   *
// Format of this parameter in the preferences si 
// fid_n = value:command 
// where n is defined from 0 to number NumButtons-1
// value is the ADC readings on the reserved pin: keyPin (35)
// command is a valid ESP32 command string or an empty string
// All buttons have to be defined to reflect all ADC values
// if there is no command leave it empty
//**************************************************************************************************
void readAnalogButtons()
{
  char        mykey[NumButtons+1];                          // For numerated key
  int8_t      pinnr;                                        // GPIO pinnumber to fill
  int         i;                                            // Loop control
  String      val;                                          // Contents of preference entry
  uint16_t    minValue;                                     // min value to be detected by pressed button
  int         indexOfCol;                                   // index of colon 

  for ( i = 0 ; ( pinnr = analogpin[i].commandNumber ) >= 0 ; i++ )    // Scan for all programmable pins
  {
    //dbgprint ( "Check programmable fid_%01d",pinnr ) ;
    sprintf ( mykey, "fid_%01d", pinnr ) ;                  // Form key in preferences
    if ( nvssearch ( mykey ) ) {
      val = nvsgetstr ( mykey ) ;                           // Get the contents
      if ( val.length() ) {                                 // Does it exists?
        indexOfCol = val.indexOf(':');                      // find delimiter
        if (indexOfCol > 0) {                               // found value (at least one digit e.g. 0:)
          minValue = val.substring(0,indexOfCol).toInt();   // try to convert found value 
          if ( minValue > 0) {                              // found something meangfull
            analogpin[i].minValue = minValue;               // Set mininimu value for the button
            val = val.substring(indexOfCol+1);              // get command, if any
            if ( val.length() ) {                           // Does it exists?
              analogpin[i].avail = true ;                   // This one is active and available now
              analogpin[i].command = val;                   // Set command
            }
            dbgprint ( "Fidelio: fid_%01d at < [%d] will execute [%s]",
                                    pinnr,
                                    analogpin[i].minValue,
                                    analogpin[i].command.c_str()); // Show result            
          }
        }
      }
    }
...
void readIOprefs()
{
  ...
  struct iosetting klist[] = {                            // List of I/O related keys
    ...
    { "pin_led_out",   &ini_block.pin_led_out,      -1 }, //  PvF: bottom leds driver
    { "pin_stb_out",   &ini_block.pin_stb_out,      -1 }, //  PvF: GPIO out STB  Fidelo Display PT6964
    { "pin_clk_out",   &ini_block.pin_clk_out,      -1 }, //  PvF: GPIO out CLK  Fidelo Display PT6964
    { "pin_dio_out",   &ini_block.pin_dio_out,      -1 }, //  PvF: GPIO out DI/O Fidelo Display PT6964
...
void  scananalog()
{
  static uint32_t oldmillis = 5000 ;                        // To compare with current time
  int             i ;                                       // Loop control
  int8_t          cmd;                                      // command number number to check
  bool            level ;                                   // Input level
  const char*     reply ;                                   // Result of analyzeCmd
  int16_t         alevel ;                                  // Level found by touch pin
  ...
  oldmillis = millis() ;                                    // 100 msec over
  // Now for the analog 4 keys of Philips DS1150
  alevel = ( analogRead ( keyPin ) ) ;                      // Sample the pin
  #ifdef TESTSCANANALOG
    dbgprint ( "Analog is now %d", alevel) ;
  #endif
  if ( alevel < NoAnalogButtonPressed ) {                  // detected any pressed key 
    i = 0;
    cmd = analogpin[i].commandNumber;
    do {
      if (alevel < analogpin[i].minValue) break;
    } while ( (cmd = analogpin[++i].commandNumber) >= 0 );
    if (cmd >= 0 && analogpin[cmd].avail) {           
      // save current values in the last list entry for execution after button will be released
      analogpin[NumButtons].cur = true;                     // there is pressed button
      analogpin[NumButtons].last_cmd = cmd;                 // which has the number command number cmd
      analogpin[NumButtons].minValue = alevel;              // and the value was alevel
    }
  } else {
      if (analogpin[NumButtons].cur) {
      analogpin[NumButtons].cur = false;
      cmd = analogpin[NumButtons].last_cmd;
      dbgprint ( "Fidelo %02d was %d ( < %d ), execute %s",
                  cmd, analogpin[NumButtons].minValue, analogpin[cmd].minValue,
                  analogpin[cmd].command.c_str() ) ;
      reply = analyzeCmd ( analogpin[cmd].command.c_str() ); // Analyze command and handle it
      dbgprint ( reply ) ;                                   // Result for debugging    
      }
  }
}
...
//                                     D R I V E P W M       T                                     *
//**************************************************************************************************
// Set and or change PWM value, update LED output pin                                              *
//**************************************************************************************************
#define FREQ 5000
#define LEDCHANNEL 0
#define RESOLUTION 8
inline u_int8_t setPWM(u_int8_t idx, u_int8_t chg) {
  idx = (idx + chg) % MAXPWMIDX;
  if ( ini_block.pin_led_out >= 0 ) {                    // PvF: LED control?
    ledcWrite(LEDCHANNEL, PWMValue[idx]);
  }
  return idx;
}
...
//**************************************************************************************************
// Show start steps on Fidelio display and print time to debug                                     *
//**************************************************************************************************

#define STARTTIME
void showStartTime(u_int32_t &t, int st) {
#ifdef STARTTIME    
  static int startIdx;
  char *ptr = NULL;
  char buf[]="   0123456789    ";
  int delta;
  u_int32_t cur = millis();

  if (0 == st) {
    startIdx = 0;
    PWMidx = setPWM(MAXPWMIDX-1, 0);
    delta = 1;
  } else if (1 == st) {
    startIdx += 1;
    delta = 1;
    ...
    startIdx += 1;
    delta = strlen(buf)-startIdx-3;
  } else {
    return;
  }
  dbgprint ( "Step: %02d -> duration %d", startIdx, cur-t) ; // show duration  
  t = cur;
  #ifdef FIDELO
    if ( ini_block.pin_stb_out >= 0 )                       // PvF: enbale fidelio display
      for (int i=startIdx; i < startIdx+delta; i++) {
        ptr = buf+i;
        fideDisplay.print(ptr);
        if (-1 == st) delay(200);
      }
    if (-1 == st) {
      fideDisplay.cls();
...
void setup()
{
  ...
  delay ( 500 ) ;                                        // PvF: should be enough for serial and PlatformIO monitor to start  
  Serial.println() ;
  ...
  #ifdef FIDELO
    dbgprint ("Fidelo defined" );                        // PfV: Did you define Fidelo?
  #endif
  ...
  #endif
  #ifdef SCANANALOG
    readAnalogButtons();                                 // Read definiotn for analog buttons
  #endif
  ...
  if ( ini_block.pin_led_out >= 0 )                      // PvF: LED control?
  {
    ...
    ledcSetup(LEDCHANNEL, FREQ, RESOLUTION);             // Yes, enable PWM output
    ledcAttachPin(ini_block.pin_led_out, LEDCHANNEL);
    PWMidx = setPWM(0,0);                                // Switch off during start
  }

  if ( ini_block.pin_stb_out >= 0 ) {                    // PvF: enbale fidelio display
    #ifdef FIDELO
      fideDisplay.init(ini_block.pin_dio_out, ini_block.pin_clk_out, ini_block.pin_stb_out, spiClk);
      fideDisplay.draw(0, 64); fideDisplay.draw(1, 64); fideDisplay.draw(2, 64); fideDisplay.draw(3, 64); // ----
    #endif
  }

  showStartTime(startTime, 0);
  ...
  if ( dsp_ok )                                          // Init okay?
  {
    ...
    dsp_println ( tmpstr ) ;
    dsp_println ( "Ed Smallenburg & PvF" );              // and some modification from PvF :)
  ...
  #ifndef ETHERNET
    mk_lsan() ;                                          // Make a list of acceptable networks in preferences.
...
void chk_enc()
{
  ...
  {
    if ( enc_inactivity > 50 && datamode != STOPPED)          // No, more than 5 seconds inactive
    {
      enc_inactivity = 0 ;
      enc_menu_mode = VOLUME ;                                // Return to VOLUME mode
      tftset ( 1, icystreamtitle);                            // PfV: show curent stream titel
      // tftset ( 2, icyname ) ;                                 // PfV: Restore screen segment bottom part
      tftset ( 2,  getPresetName(presetinfo.preset, tmp)?tmp:icyname) ;
      dbgprint ( "Encoder mode back to VOLUME" ) ;
      // dbgprint ( "Titel: %s", icystreamtitle.c_str()) ;
    }
  }
  ...
  if ( tripleclick )                                          // First handle triple click
  {
    ...
    #ifdef FIXEDME
    dbgprint ( "Triple click" ) ;                             // PvF: fix me, quick change OTA
    ota = !ota;
    if (ota) { 
      tftset (0, "OTA=On!");                                  // Info OTA on/ready
    } else {
      tftset (0, NAME);                                       // Info OTA off
    }
    dbgprint ( "OTA: %s", ota?" On":"Off");                       
    #endif

    tripleclick = false ;                                     // Reset flag
    #ifdef SDCARD                                             // I do not use SD CARD at all, fix me
  ...
  if ( doubleclick )                                          // Handle the doubleclick
  {
    ...
    tftset ( 1, "Turn to select\n"                            // Show current option 
                "Press to confirm" ) ;
    enc_preset = presetinfo.preset ;                          // Start with current preset
    updateNr ( &enc_preset, presetinfo.highest_preset,        // plus 1
               1, true ) ;
    getPresetName(enc_preset, tmp);
    tftset ( 3, tmp );                                                     
    #ifdef FIDELO                                             // PvF: Fidelio active?
      fideDisplay.updateStation(enc_preset);                  // PvF: Show selected station number
      fideDisplay.setStartDelay(millis());                    // PvF: initiate delay of going back to time display
    #endif  
  }
  if ( singleclick )
  {
    ...
    switch ( enc_menu_mode )                                  // Which mode (VOLUME, PRESET)?
    {
      ...
      case PRESET :
        ...
        muteflag = false;                                     // just in case it was off
...
/*  myQueueSend ( sdqueue, &stopcmd ) ;                       // Stop player
    ...
    setdatamode(STOPPED);                                     // PfV: there is no data more
    displayStop();                                            // PfV: Update info on display
*/
    //if ( datamode != STOPPED )
    //{
  ...
  switch ( enc_menu_mode )                                    // Which mode (VOLUME, PRESET, TRACK)?
  {
    case VOLUME :
      ...
      displayvolume ( ini_block.reqvol ) ;                    // PvF: imediate display update
      #ifdef FIDELO                                           // PvF: Fidelio active?
        fideDisplay.updateVolume(ini_block.reqvol);           // PvF: Volume on front display
        fideDisplay.setStartDelay(millis());                  // PvF: initiate delay of going back to time display
      #endif
      break ;
    case PRESET:
      ...
      getPresetName(enc_preset, tmp);
      tmp = tmp.substring(0, 40);                             // PvF: if there is missing a name of the stream, a v.long stream address destroys text on screen
  ...
  if ( spftrigger )                                             // Will be set every 100 msec
  {
    ...
    if ( time_req )                                             // Time to refresh timetxt?
    {
      ...
      if ( NetworkFound  )                                      // Time available?
      {
        ...
        #ifdef FIDELO
        fideDisplay.updateTime(timetxt);                        // PvF: update time on Fidelo display
        #endif
      }
    ...
    #ifdef FIDELO
      fideDisplay.setBright(
        map(
            map(4095-analogRead(dimPin), 0, 4095, 0, 4),
                                         0,    4, 0, 7)
      );                                                      // adapt Fidelio britness
      // Serial.println(analogRead(dimPin));
    #endif
...
//   Philips Fidelo new commands                                                                   *
//   toggle                                 // off/on player                                       *
//   bleds                                  // rotate brightness of bottom leds                    *
...
const char* analyzeCmd ( const char* par, const char* val )
{
  ...
  else if ( argument == "toggle" ) {                  // PvF: new command toggle radio status on/off
    if ( datamode == STOPPED ) {                      // if already stopped, start it
      if ( NetworkFound ) {                           // Start with preset if network available
        // nextPreset ( presetinfo.preset, false  ) ; // Resore last preset
        myQueueSend ( radioqueue, &startcmd ) ;       // Start player in radio mode
        sprintf ( reply, "Starting player" );
        dbgprint ( "Starting player %d", presetinfo.preset);
        String tmp = "";
        tftset ( 2,  getPresetName(presetinfo.preset, tmp)?tmp:icyname) ;
      }
    } else {                                          // ok, not stopped, then stop it
      myQueueSend ( sdqueue, &stopcmd );              // Stop player
      myQueueSend ( radioqueue, &stopcmd );           // Stop player
      setdatamode(STOPPED);                           // PfV: there is no data more
      displayStop();                                  // PfV: Update info on display
  ...
  else if ( argument == "bleds" ) {                  // PvF: new command toggle radio status on/off
    PWMidx = setPWM(PWMidx,1);
  }  
  else
  {
...
//                                      D I S P L A Y STOP                                         *
//**************************************************************************************************
// Update info on display(s) on stopped player                                                      *
//**************************************************************************************************
void displayStop()
{

    tftset ( 1, "" );                                         // PfV: Update info on display
    tftset ( 2, "" );                                         // PfV: clear screen
    tftset ( 3, "Player OFF" ) ;                              // PvF: Player off on display
    #ifdef FIDELO
      fideDisplay.alarm(true);                                // PvF: in Stop mode turn on alarm LED
    #endif
}
